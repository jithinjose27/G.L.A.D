...
class Config:
    ...
    WAKE_WORDS = [
        "hello robo", "hi robo", "hey robo", "hello glad", "hey glad",
        "hi glad", "hi robo", "hello dad", "hey dad", "hey lad", "hi lad"
    ]
    
    # Words that put the robot back to sleep
    SLEEP_WORDS = [
        "thank you", "thanks", "stop glad", "go to sleep", 
        "thank you robo", "bye", "thanks robo", "quiet"
    ]
...
def speak(text, print_text=True):
    """Plays audio instantly using local TTS. No internet delay!"""
    if print_text:
    ...
    try:
        # -ven+f3: Female English voice (you can change f3 to m1 for male)
        # -s 160: Speed of the voice
        # -a 200: Volume amplitude
        subprocess.run(
            ["espeak", "-ven+f3", "-s", "160", "-a", "200", clean_text], 
...
class VoiceAssistantNode(Node):
    def __init__(self):
        ...
        print(f"🔧 Hardware Detected -> Mic Index: {Config.MIC_INDEX} | Speaker ID: {Config.SPEAKER_HW}")
    ...
    def run_voice_loop(self):
        ...
        while rclpy.ok():
            try:
                prompt_msg = "Listening for a question..." if self.is_awake else "Waiting for wake word ..."
