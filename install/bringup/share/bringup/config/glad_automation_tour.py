...
def find_usb_speaker():
    ...
    try:
        ...
        for line in out.split('\n'):
            if "card" in line.lower() and ("YM USB Audio Device" in line or "USB Audio" in line):
...
def generate_speech(client, prompt):
    ...
    if not client: return "I am sorry, my AI brain is disconnected."
...
def speak(text):
    ...
    try:
        ...
        subprocess.run(["mpg123", "-a", SPEAKER_HW, "-q", temp_mp3], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def main():
    ...
    waypoints = [
        {
            'name': 'Point 1', 'x': 0.483, 'y': 0.24, 'w': 1.0, 
            'prompt': "We have just arrived at the main Automation Lab. Greet the guests and give a brief overview of automation lab."
        },
        {
            'name': 'Point 2', 'x': 2.69, 'y': 0.428, 'w': 1.0,
            'prompt': "We are now standing in front of the hydraulic system. Briefly explain what an industrial hydraulic system is and why students learn to use it."
        },
        {
            'name': 'Point 3', 'x': 3.1, 'y': -2.56, 'w': 1.0,
            'prompt': "We are now standing in front of the pneumatic system. Briefly explain what an industrial pneumatic system is and why students learn to use it."
        }
    ]
    ...
    for point in waypoints:
        ...
        if result == TaskResult.SUCCEEDED:
            ...
            speak("I am sorry, my path is blocked. I cannot reach the next destination.")
            break 
